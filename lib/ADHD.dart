...
class FocusTimerService extends ChangeNotifier {
  ...
  int _remainingSeconds = 25 * 60; // Initialize with default time
  ...
  void setDurationInSeconds(int seconds) {
    if (!_isRunning && !_isPaused) {
      _selectedMinutes = 1; // Set to 1 minute for display purposes
  ...
  void startTimer() {
    if (_isPaused) {
      ...
      // Start new timer - ensure we have a valid time
  ...
  void resetTimer() {
    _timer?.cancel();
    // Ensure we reset to a valid time
    if (_selectedMinutes > 0) {
      _remainingSeconds = _selectedMinutes * 60;
    } else {
      // Fallback to default 25 minutes if somehow selectedMinutes is 0
      _selectedMinutes = 25;
...
class Consumer<T extends ChangeNotifier> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final service = context.dependOnInheritedWidgetOfExactType<_InheritedProvider<T>>();
...
class _ADHDPageState extends State<ADHDPage> {
  ...
  Future<void> _loadUploadedDocuments() async {
    ...
    if (documentsJson != null) {
      ...
      setState(() {
        uploadedDocuments = documentsList.map((doc) => Map<String, String>.from(doc)).toList();
      });
    }
  }
  ...
  Future<void> _uploadDocument() async {
    try {
      ...
      if (result != null && result.files.single.path != null) {
        ...
        String aiSummary = 'Document uploaded successfully. Use Summary and Flash Cards buttons below.';
        String aiInsights = 'Click the buttons below to generate ADHD-optimized content.';
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Document "$fileName" uploaded and processed successfully!'),
  ...
  Future<String> _extractFileContent(File file, String extension) async {
    try {
      print('Extracting content from file: ${file.path}, extension: $extension');
      
      switch (extension.toLowerCase()) {
        case 'txt':
          final content = await file.readAsString();
          print('TXT file content length: ${content.length}');
          print('TXT file content preview: ${content.length > 100 ? content.substring(0, 100) : content}...');
          return content;
        case 'pdf':
          // For PDF files, we'll send a message about the limitation
          final message = 'This is a PDF file named "${file.path.split('/').last}". Please provide a general analysis based on the filename and note that full content extraction from PDF files requires additional processing. Create educational content based on what the filename suggests this document might contain.';
          print('PDF message: $message');
          return message;
        case 'docx':
          // For DOCX files, we'll send a message about the limitation
          final message = 'This is a DOCX file named "${file.path.split('/').last}". Please provide a general analysis based on the filename and note that full content extraction from DOCX files requires additional processing. Create educational content based on what the filename suggests this document might contain.';
          print('DOCX message: $message');
          return message;
        default:
  ...
  Future<String> _generateADHDSummary(String content, String fileName) async {
    try {
      ...
      print('Content preview: ${content.length > 200 ? content.substring(0, 200) : content}...');
...
''';
      ...
      final result = response.text ?? 'Unable to generate summary at this time.';
  ...
  Future<List<Map<String, String>>> _generateFlashCards(String content, String fileName) async {
    try {
      final prompt = '''
...
''';
      ...
      final cardMatches = RegExp(r'CARD_START(.*?)CARD_END', dotAll: true).allMatches(responseText);
      
      for (final match in cardMatches) {
        final cardContent = match.group(1) ?? '';
        final questionMatch = RegExp(r'QUESTION:\s*(.*?)(?=ANSWER:|$)', dotAll: true).firstMatch(cardContent);
        final answerMatch = RegExp(r'ANSWER:\s*(.*?)$', dotAll: true).firstMatch(cardContent);
        ...
        flashCards = [
          {
            ...
            'answer': 'This document covers important information that has been processed for ADHD learning.',
          },
        ];
      }
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    Expanded(
                      child: _buildToolCard(
                        ...
                        title: timerService.isActive 
                          ? 'Focus Timer (${timerService.formatTime(timerService.remainingSeconds)})'
                          : 'Focus Timer',
                        subtitle: timerService.isRunning 
                          ...
                          : timerService.isPaused 
                            ? 'Paused' 
                            : 'Pomodoro technique',
                        color: timerService.isActive ? Colors.green : Colors.blue,
                    ...
                    Expanded(
                      child: _buildToolCard(
                        icon: Icons.task_alt,
                        title: 'Task Planner',
                        subtitle: 'Break down tasks',
                        color: Colors.orange,
                        onTap: () => _showTaskPlanner(context),
                      ),
                    ),
                  ],
                ),
                ...
                _buildTipCard(
                  ...
                  tip: 'Create a quiet, organized space with minimal distractions. Use noise-cancelling headphones if needed.',
                  icon: Icons.desk,
                ),
                const SizedBox(height: 12),
                _buildTipCard(
                  title: 'Time Management',
                  tip: 'Use timers and break tasks into smaller, manageable chunks. The Pomodoro Technique works well.',
                  icon: Icons.access_time,
                ),
                const SizedBox(height: 12),
                _buildTipCard(
                  title: 'Note Taking',
                  tip: 'Use visual aids, mind maps, and color coding to make information more engaging and memorable.',
                  icon: Icons.note_alt,
                ),
                const SizedBox(height: 24),
              ],
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              _buildProgressStat('Completed', '12', Colors.green),
              _buildProgressStat('Total Time', '3h 45m', Colors.blue),
              _buildProgressStat('Streak', '5 days', Colors.orange),
            ],
          ),
          const SizedBox(height: 16),
  ...
  void _showTaskPlanner(BuildContext context) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Task planner feature coming soon! This will help you break down complex tasks into manageable steps.'),
        actions: [
          TextButton(
  ...
  void _showMindfulness(BuildContext context) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Mindfulness exercises coming soon! This will include breathing exercises and meditation guides.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('OK'),
  ...
  void _showFocusSounds(BuildContext context) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Focus sounds feature coming soon! This will provide background sounds to help with concentration.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('OK'),
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (aiSummary.length > 150 || aiInsights.isNotEmpty)
                    TextButton(
                      onPressed: () => _showAIAnalysis(context, fileName, aiSummary, aiInsights),
          ...
          if (aiSummary.isNotEmpty) ...[
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: () => _generateAndShowFlashCards(context, document),
                    icon: const Icon(Icons.quiz, size: 16),
                    label: const Text('Flash Cards', style: TextStyle(fontSize: 12)),
                    style: ElevatedButton.styleFrom(
  ...
  void _showDeleteConfirmation(BuildContext context, int index, String fileName) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
  ...
  void _showAIAnalysis(BuildContext context, String fileName, String summary, String insights) {
    showDialog(
      context: context,
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Icon(Icons.summarize, size: 20, color: Colors.blue[600]),
                                  const SizedBox(width: 8),
                                  Text(
                                    'Summary',
                                    style: TextStyle(
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Icon(Icons.lightbulb, size: 20, color: Colors.green[600]),
                                  const SizedBox(width: 8),
                                  Text(
                                    'ADHD Study Insights',
                                    style: TextStyle(
  ...
  Future<void> _generateAndShowSummary(BuildContext context, Map<String, String> document) async {
    final fileName = document['name'] ?? '';
    ...
    try {
      ...
      final content = await _extractFileContent(file, document['extension'] ?? '');
  ...
  Future<void> _generateAndShowFlashCards(BuildContext context, Map<String, String> document) async {
    final fileName = document['name'] ?? '';
    final filePath = document['path'] ?? '';
    ...
    try {
      ...
      // Show flash cards
      _showFlashCards(context, fileName, flashCards);
    } catch (e) {
      // Close loading dialog
      if (mounted) Navigator.pop(context);
  ...
  void _showADHDSummary(BuildContext context, String fileName, String summary) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ADHDSummaryScreen(
          fileName: fileName,
          summary: summary,
        ),
      ),
  ...
  // Show flash cards
  void _showFlashCards(BuildContext context, String fileName, List<Map<String, String>> flashCards) {
    Navigator.push(
      context,
      MaterialPageRoute(
...
class _FocusTimerScreenState extends State<FocusTimerScreen> with TickerProviderStateMixin {
  ...
  void _initializeAnimations() {
    ...
    _flashAnimation = ColorTween(
      ...
      end: const Color(0xFF4CAF50).withOpacity(0.3),
  ...
  void _onTimerUpdate() {
    if (_timerService.remainingSeconds == 0 && !_timerService.isRunning && !_timerService.isPaused) {
      _triggerScreenFlash();
  ...
  String _getSelectedTimeLabel() {
    final selected = _timeOptions.firstWhere(
      (option) => (option['minutes'] ?? (option['seconds'] ?? 0) ~/ 60) == _timerService.selectedMinutes,
                        ...
                        children: [
                          ...
                          if (!timerService.isRunning && !timerService.isPaused) ...[
                            ...
                            Wrap(
                              ...
                              children: _timeOptions.map((option) {
                                final minutes = option['minutes'] ?? (option['seconds'] ?? 0) ~/ 60;
                                final seconds = option['seconds'];
                                final label = option['label'];
                                final isSelected = minutes == timerService.selectedMinutes;
                                
                                return GestureDetector(
                                  onTap: () {
                                    if (seconds != null) {
                                      // Handle seconds (for testing)
                                      timerService.setDurationInSeconds(seconds);
                                    } else {
                                            ...
                                            children: [
                                              ...
                                              Center(
                                                child: SizedBox(
                                                  ...
                                                  child: CircularProgressIndicator(
                                                    value: timerService.selectedMinutes > 0
                                                      ? (timerService.selectedMinutes * 60 - timerService.remainingSeconds) / (timerService.selectedMinutes * 60)
                                                      : 0.0,
                                                    strokeWidth: 8,
                                                    backgroundColor: Colors.grey[200],
                                              ...
                                              Center(
                                                child: Column(
                                                  ...
                                                  children: [
                                                    Text(
                                                      timerService.formatTime(timerService.remainingSeconds),
                                                      style: TextStyle(
                                                        fontSize: 48,
                                                        fontWeight: FontWeight.bold,
                                                        color: timerService.remainingSeconds == 0 
                                                          ? const Color(0xFF4CAF50)
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              ElevatedButton.icon(
                                onPressed: (!timerService.isRunning || timerService.isPaused || timerService.remainingSeconds > 0)
                                  ? (timerService.isRunning ? () => timerService.pauseTimer() : () => timerService.startTimer())
                                  : null,
                                icon: Icon(
                                ...
                                label: Text(
                                  timerService.isRunning ? 'Pause' : (timerService.isPaused ? 'Resume' : 'Start'),
                                  style: const TextStyle(fontSize: 16),
                                ),
                                style: ElevatedButton.styleFrom(
                                  backgroundColor: const Color(0xFF4CAF50),
...
class ADHDSummaryScreen extends StatelessWidget {
  ...
  Widget _buildInlineFormattedText(String text) {
    ...
    final markers = RegExp(r'\|\|\|(BOLD|ITALIC|BOLDITALIC)\|\|\|').allMatches(text).toList();
              ...
              children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color: isFlipped ? Colors.green[50] : Colors.blue[50],
                                  borderRadius: BorderRadius.circular(20),
                                ),
                                child: Text(
                                  ...
                                  style: TextStyle(
                                    fontSize: 12,
                                    fontWeight: FontWeight.bold,
                                    color: isFlipped ? Colors.green[700] : Colors.blue[700],
                                  ),
                                ),
                              ),
                              const SizedBox(height: 20),
                ...
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton.icon(
                      onPressed: _flipCard,
                      icon: Icon(isFlipped ? Icons.quiz : Icons.help_outline, size: 18),
                    ...
                    ElevatedButton.icon(
                      onPressed: currentCardIndex < widget.flashCards.length - 1 ? _nextCard : null,
